//! Dashboard layout & list commands (per-project layout, list available, templates, etc.).
...
use libretune_core::dashboard::{get_dashboard_file_path, DashboardLayout};
use serde::Serialize;
use std::path::Path;
...
#[tauri::command]
pub async fn list_dashboard_layouts(
    _state: tauri::State<'_, AppState>,
